...
SherpaNcnnTtsAudio* SherpaNcnnTtsGenerate(SherpaNcnnTts* ntts, const char* text,
  ...
  auto callback = config->callback ? config->callback : DefaultAudioCallback;
  auto audio = ntts->tts->Generate(args, callback, config->user_data);
...
uint32_t SherpaNcnnTtsAudioGetSampleRate(SherpaNcnnTtsAudio* audio) {
  return audio->sample_rate;
}

float* SherpaNcnnTtsAudioGetSamplePtr(SherpaNcnnTtsAudio* audio) {
  return audio->audio->samples.data();
}

uint32_t SherpaNcnnTtsAudioGetSampleCount(SherpaNcnnTtsAudio* audio) {
  return audio->nsamples;
}
float SherpaNcnnTtsAudioGetDuration(SherpaNcnnTtsAudio* audio) {
  return audio->duration;
}

float SherpaNcnnTtsAudioGetRtf(SherpaNcnnTtsAudio* audio) { return audio->rtf; }

float SherpaNcnnTtsAudioGetElapsedSeconds(SherpaNcnnTtsAudio* audio) {
